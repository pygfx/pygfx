...
def fragment_shader_textured_gray(
    ...
    val = f32(t_tex.sample(s_sam, v_texcoord).r) / 255.0
...
def mesh_renderer(wobject, render_info):
    ...
    if getattr(material, "texture", None) is not None:
        ...
        bindings0[1] = (wgpu.BindingType.sampler, material.texture)
        bindings0[2] = (wgpu.BindingType.sampled_texture, material.texture)
        if "rgba" in material.texture.format:
