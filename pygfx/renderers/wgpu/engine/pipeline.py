...
def get_pipeline_container_group(wobject, scene, renderer, renderstate):
    ...
    pipeline_container_group.update(wobject, scene, renderer, renderstate)
...
class PipelineContainerGroup:
    ...
    def update(self, wobject, scene, renderer, renderstate):
        ...
        if changed:
            for container in self.compute_containers:
                container.update(
                    wobject, scene, renderer, renderstate, changed, tracker
                )
...
class PipelineContainer:
    ...
    def update(self, wobject, scene, renderer, renderstate, changed, tracker):
        ...
        if changed:
            try:
                self.update_shader_data(wobject, scene, renderer, changed, tracker)
    ...
    def update_shader_data(self, wobject, scene, renderer, changed, tracker):
        ...
        if "bindings" in changed:
            with tracker.track_usage("!bindings"):
                self.bindings_dicts = self.shader.get_bindings_info(
                    wobject, self.shared, scene, renderer
                )
