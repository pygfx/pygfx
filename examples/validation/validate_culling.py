...
def create_scene(title):
    ...
    obj1.transform.rotation = la.quaternion_multiply(rot, obj1.transform.rotation)
    obj2.transform.rotation = la.quaternion_multiply(rot, obj2.transform.rotation)

    t = gfx.Text(
        gfx.TextGeometry(title, screen_space=True, font_size=20), gfx.TextMaterial()
    )
    t.transform.position = (0, 4, 0)
...
scene2.children[0].transform.scale_x = -1
scene2.children[1].transform.scale_x = -1

vp3 = gfx.Viewport(renderer, (600, 0, 300, 600))
scene3 = create_scene("Rotate camera")
transform = scene3.children[-1].transform
